...
// PostgreSQL kütüphanesi (pg) kullanıyoruz
const { Pool } = require('pg'); 
const cors = require('cors'); 
const jwt = require('jsonwebtoken');

const app = express();
// Canlı ortamda PORT'u Render belirler, yerel için 3000
...
// Canlı ortamda Render'ın verdiği DATABASE_URL'i kullanır
const dbConfig = {
    connectionString: process.env.DATABASE_URL,
    // SSL zorunlu: Render'a yüklerken bu ayar gereklidir.
...
if (process.env.NODE_ENV !== 'production') {
    dbConfig.ssl = false;
    // Yerel MySQL bilgilerinizi buraya elle girebilirsiniz (şimdilik PostgreSQL ile devam edelim)
}

// Veritabanı bağlantı havuzu
let pool = new Pool(dbConfig); 

// PostgreSQL'e özel, tek bir tablo oluşturma fonksiyonu (Yalnızca veritabanı boşsa çalışır)
    ...
    try {
        ...
        const schema = `
            ...
            CREATE TABLE IF NOT EXISTS CevapSiklari (
                ...
                sik_metni VARCHAR(255) NOT NULL,
        ...
        // Admin kullanıcısı yoksa ekle (PostgreSQL'e uyarlanmıştır)
...
// Tüm rotaların çalışabilmesi için setupDatabase fonksiyonunu çağırmalıyız.
setupDatabase();
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            "INSERT INTO Kullanicilar (email, sifre_hash, rol) VALUES ($1, $2, 'kullanici') RETURNING kullanici_id",
            [email, password]
        );
...
app.post('/api/admin/add-question', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        try {
            // PostgreSQL'de RETURNING kullanılır
            const result = await client.query(
                'INSERT INTO Sorular (soru_metni, fotograf_url, dogru_cevap_index) VALUES ($1, $2, $3) RETURNING soru_id',
            ...
            for (let i = 0; i < cevap_siklari.length; i++) {
                await client.query(
                    'INSERT INTO CevapSiklari (soru_id, sik_metni, sik_index) VALUES ($1, $2, $3)',
                    [newSoruId, cevap_siklari[i], i + 1]
                );
            }
    ...
    try {
        ...
        for (const question of questions) {
            const choicesResult = await pool.query(
                'SELECT sik_index, sik_metri FROM CevapSiklari WHERE soru_id = $1 ORDER BY sik_index',
                [question.soru_id]
            );
...
app.post('/api/quiz/submit', authenticateToken, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO QuizSonuclari (kullanici_id, dogru_sayisi, yanlis_sayisi, sure_saniye) VALUES ($1, $2, $3, $4) RETURNING sonuc_id',
            [kullanici_id, dogru_sayisi, yanlis_sayisi, sure_saniye]
        );
